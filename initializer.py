...
#                omegaU=self.Cfg.omgRange[1],energy=self.Cfg.energy)
    ...
    def simMap(self,tmpxx,tmpyy,AllMaxS,blur=False,dtype=np.uint32):
        ...
        for ii in range(self.NumG):
            ...
            myMaps=np.zeros((45,self.LimH[ii][3]-self.LimH[ii][2],self.LimH[ii][1]-self.LimH[ii][0]),dtype=dtype)
            for jj in range(45):
                ...
                tmp=coo_matrix((val,(yy,xx)),
                        shape=(self.LimH[ii][3]-self.LimH[ii][2],self.LimH[ii][1]-self.LimH[ii][0])).toarray()
                if blur:
